...
import os
from transformers import PerceiverConfig, PerceiverModel, PerceiverTokenizer, PerceiverFeatureExtractor
...
# ===== 2. Updated Model Architecture with Pre-trained Perceiver IO =====
class PreTrainedPerceiverMultiModal(nn.Module):
    def __init__(self, text_dim=768, audio_dim=1024, video_dim=2048, num_classes=3):
        super().__init__()
        ...
        # Create a custom Perceiver config instead of loading pre-trained
        self.config = PerceiverConfig()
        self.config.num_labels = num_classes
        self.config.d_model = 768  # Keep the original d_model from pre-trained
        self.config.num_self_attention_layers = 3
        self.config.num_cross_attention_layers = 2
        
        # Initialize a new model with our custom config
        self.perceiver = PerceiverModel(self.config)
        
        # Projection layers to match expected input dimension
        self.text_proj = nn.Linear(text_dim, self.config.d_model)
        self.audio_proj = nn.Linear(audio_dim, self.config.d_model)
        self.video_proj = nn.Linear(video_dim, self.config.d_model)
    ...
    def forward(self, text_feat, audio_feat, video_feat):
        ...
        # Project each modality to match model's input dimension
        text_hidden = self.text_proj(text_feat)
        audio_hidden = self.audio_proj(audio_feat)
        video_hidden = self.video_proj(video_feat)
        
        # Concatenate all modalities
        # Shape: [batch_size, 3*seq_len, d_model]
        combined_input = torch.cat([text_hidden, audio_hidden, video_hidden], dim=1)
        
        # Create attention mask for valid inputs
        ...
        hidden_states = outputs.last_hidden_state  # This is likely [batch_size, latent_size, d_model]
        
        # Instead of reshaping, we need to use a different approach
        # Let's use average pooling to get a fixed-size representation
        hidden_states = hidden_states.mean(dim=1)  # [batch_size, d_model]
        
        # Expand to match the expected sequence length
        hidden_states = hidden_states.unsqueeze(1).expand(-1, seq_len, -1)  # [batch_size, seq_len, d_model]
...
def train_model(csv_file, feature_dir, training_params):
    ...
    model = PreTrainedPerceiverMultiModal(**model_params).to(device)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if best_model_files:
            ...
            model = PreTrainedPerceiverMultiModal(**saved_model_params).to(device)
